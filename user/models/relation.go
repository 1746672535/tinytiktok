...
type Relation struct {
	...
	UserID int64 `gorm:"column:user_id" json:"user_id"` // 自己
...
func GetFollowList(db *gorm.DB, userId int64) []*Relation {
	...
	result := db.Where("user_id=? and state=1", userId).Find(&relation)
	if result.Error != nil {
		return nil
	}
	return relation
...
func IsMutualFollow(db *gorm.DB, userId, pId int64) bool {
	var relation Relation
	result := db.Where("user_id=? and pid=? and state= 1", userId, pId).Find(&relation)
