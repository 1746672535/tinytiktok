...
func (messageService *MessageServiceImpl) SendMessage(fromUserId int64, toUserId int64, content string, actionType int64) error {
	...
	// actionType = 1 发送消息
	case 1:
		err = SendMessage(fromUserId, toUserId, content, actionType)
	default:
...
func (messageService *MessageServiceImpl) MessageChat(loginUserId int64, targetUserId int64, latestTime time.Time) ([]Message, error) {
	...
	plainMessages, err := MessageChat(loginUserId, targetUserId, latestTime)
	if err != nil {
...
func (messageService *MessageServiceImpl) LatestMessage(loginUserId int64, targetUserId int64) (LatestMessage0, error) {
	plainMessage, err := LatestMessage1(loginUserId, targetUserId)
	if err != nil {
		log.Println("LatestMessage Service:", err)
		return LatestMessage0{}, err
	}
	var latestMessage LatestMessage0
...
func (messageService *MessageServiceImpl) getRespMessage(messages *[]Message, plainMessages *[]Message) error {
...
func (messageService *MessageServiceImpl) combineMessage(message *Message, plainMessage *Message) error {
